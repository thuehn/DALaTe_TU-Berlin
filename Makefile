...
version		:= 2.2.0
#
# lets add our local style & Bibliography & package folder to the project (*.sty, *.cls, *.bst)
TEXINPUTS:=.:local_tex_packages:
BSTINPUTS:=.:local_tex_packages:
BIPINPUTS:=.:bibliography:
export TEXINPUTS BSTINPUTS BIPINPUTS
#
.DEFAULT_GOAL	:= all
...
files.R		:= $(call filter-buildable,R)
...
default_files.R		:= $(call filter-default,R)
...
all_stems.R		:= $(call get-stems,R,all)
...
default_stems.R			:= $(call get-stems,R,default)
...
# Converts R files into .pdf files
#
# $(call convert-svg,<svg file>,<eps/pdf file>,[gray])
convert-gnur	= cd $(<D) ; $(GNUR) --vanilla -q --slave < `basename $1` > /dev/null
